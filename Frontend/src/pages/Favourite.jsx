...
const Favourite = () => {
  ...
  // Fetch favourites from backend on mount or token change
  useEffect(() => {
    if (!token) return;

    const fetchFavourites = async () => {
      try {
        const res = await axios.get(urls.favourites, {
  ...
  // Remove movie from favourites on backend & update state
  const removeFavourite = async (movieId) => {
    try {
      await axios.delete(`${urls.favourites}/${movieId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
