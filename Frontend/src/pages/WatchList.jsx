...
const Watchlist = () => {
  ...
  // Extract token from user (adjust if your user object is different)
  ...
  // Fetch watchlist from backend on mount or token change
  useEffect(() => {
    if (!token) return;

    const fetchWatchlist = async () => {
      try {
        const res = await axios.get(urls.watchlist, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setWatchlist(res.data.watchlist || []);
      } catch (error) {
        console.error("Failed to fetch watchlist:", error);
        // Optional: show toast or notification for error
      }
    };
  ...
  // Remove movie from watchlist on backend & update state
  const removeWatchlist = async (movieId) => {
    try {
      await axios.delete(`${urls.watchlist}/${movieId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    ...
    >
            ...
            <DropdownMenuContent className="bg-white text-black">
              {isLoggedIn ? (
                ...
                <>
                  ...
                  <DropdownMenuItem onClick={() => navigate("/login", { state: { tab: "register" } })}>
                    Register
                  </DropdownMenuItem>
                </>
              )}
      ...
      <div className="px-6 py-10 min-h-screen bg-black/60">
        <h2 className="text-4xl font-bold text-center mb-10">👁️ Your Watchlist</h2>
        {watchlist.length === 0 ? (
          <p className="text-3xl text-gray-400 text-center mt-20">No movies added to watchlist</p>
        ) : (
